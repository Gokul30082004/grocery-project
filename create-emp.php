...
<body>
	...
	<div id="main-area">
		<div id="sec1">
			<div style="margin-top: 10px;">
				...
				<div id="logo-txt">
					...
					<h1 style="color: #4318FF;">Creat New Employee</h1>
				</div>
			</div>
		</div>
		<div id="sec2">
		...
		<div id="sec4">
			<div style="margin-top: 10px;height: 59px;" id="profile-header">
	...
	<div id="tassign">
	<div class='assign-ticket'><center><h3 style='display:block;font-weight: 400;font-size: 30px;line-height:2;color: #A3AED0;'>Create Employee Id</h3></center><form action='' method='POST' class='leave-form' id='leave-form'><label style='display:block;'>Employee Name</label><input  placeholder='Employee name' type='text' name='ename' class='leave-reason' id='leave-reason' form='leave-form' required><br><label>Salary</label><br><input type='tel' name='esalary' class='leave-reason' id='leave-reason' form='leave-form' placeholder='Salary' required><br><label style='display:block;'>Account Number</label><input  placeholder='Account no' type='tel' name='eaccount' class='leave-reason' id='leave-reason' form='leave-form' required><br><label style='display:block;'>Address</label><input  placeholder='Address' type='text' name='eaddress' class='leave-reason' id='leave-reason' form='leave-form' required><br><label style='display:block;'>Phone Number</label><input  placeholder='Phone no' type='tel' name='ephn' class='leave-reason' id='leave-reason' form='leave-form' required><button type='submit' form='leave-form' value='Apply' name='submit' class='apply-leave-btn'>Create</button></div></form></div></div>
	</div>
</body>
<?php
if(isset($_POST['submit']))
{
	...
	$sql="INSERT INTO `emp`(`ename`, `edept`, `ephn`, `eaccount`, `eaddress`, `esalary`) VALUES ('$ename','Sales','$ephn','$eaccount','$eaddress','$esalary')";
	$conn->query($sql);
	echo"<script>window.alert('Inserted');</sript>";
}?>
<script>
    var stat="hide";
    functdion show_hide()
    ...
    function days_calc()
	{
	    ...
	    Diff_in_Days= tdate.getTime()-today.getTime();
	    Diff_in_Days = Diff_in_Days/(1000 * 3600 * 24);
	    Diff_in_Days++;
	    if(Diff_in_Days<1)
		{
		...
		{
			document.getElementById("no_days").innerHTML= "No.of Days: "+Math.trunc(Diff_in_Days);
		}
		 Diff_in_Days=Math.trunc(Diff_in_Days);
	}
