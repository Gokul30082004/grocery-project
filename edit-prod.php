...
<body>
  ...
  <div>
              ...
              <?php $i=$_GET['id'];
              $sql="select * from product where pname='$i'";
              $result=$conn->query($sql);
              if($prod=$result->fetch_assoc())
              {
                echo"<header style='font-weight: 400;font-size: 30px;line-height: 1.5;color: #A3AED0;'><span id='yourp'>Product details</span></header><form action='' method='POST'><input type='button' onclick='edit()' class='edit-profile-button' style='margin-top: 0%;float:right;width:60px;font-size:10px;' value='Edit Profile' id='edtprof'><br><br><table class='edit-profile-table'><tr><td><span>Product Name:     </span></td><td><input type='text' value=".$prod['pname']." readonly name='pname'id='ename'></td></tr><tr><td><span>Product amount:     </span></td><td><input readonly name='pamount' type='text' value=".$prod['pamount']." id='edept'></td></tr><tr><td><span>Profit:     </span></td><td><input readonly id='eemail' type='text' value=".$prod['pprofit']." name='pprofit'></td></tr></table><br><button type='submit' value='Save' name='submit' class='edit-profile-button-save'>Save</button><br><br></form>";
              }
              ?>
				</div>
        <?php
        if(isset($_POST['submit']))
        {
          ...
          $sql="UPDATE `product` SET `pname`='$pname',`pamount`='$pamount',`pprofit`='$pprofit' WHERE pname='$i'";
          $result=$conn->query($sql);
          echo"<script>window.alert('Updated');window.location.href='product.php';</script>";
        }
        ?> 
		    </div>
          </body>
